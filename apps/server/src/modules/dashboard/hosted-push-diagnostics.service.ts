...
export class HostedPushDiagnosticsService {
  ...
  async listTenantUserNotificationDevices(tenantId: string, userId: string) {
    ...
    const canonicalTenantId =
      await this.runtimeTenantScopeResolverService.resolveTenantIdForDiagnostics({
        tenantScope: tenantId,
        userId: normalizedUserId,
      });

    return this.repository.listNotificationDevicesForTenantUser(
      canonicalTenantId,
      normalizedUserId,
    );
  }
  ...
  async triggerTenantDeviceTestPush(tenantId: string, dto: InternalDevicePushTestDto) {
    ...
    let workerResult: Awaited<ReturnType<PushDeliveryWorkerService['runOnceForDelivery']>> | null =
      null;
    if (runWorkerOnce) {
    ...
    const delivery = await this.repository.getPushDeliveryById(
      queued.deliveryId,
      canonicalTenantId,
    );
    return {
