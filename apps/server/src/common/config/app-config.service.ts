...
export class AppConfigService {
  ...
  get releaseVersion(): string {
    return (
      this.configService.get<string>('TASKBANDIT_RELEASE_VERSION', '').trim() ||
      this.readRepositoryVersionFile()
    );
  }
  ...
  get reverseProxyEnabled(): boolean {
    const explicitValue = this.configService
      .get<string>('TASKBANDIT_REVERSE_PROXY_ENABLED')
      ?.trim();
    if (explicitValue) {
    ...
    return (
      this.isRemotePublicUrl(this.publicWebBaseUrl) || this.isRemotePublicUrl(this.publicApiBaseUrl)
    );
  }
  ...
  get reverseProxyPathBase(): string {
    const rawValue = this.configService
      .get<string>('TASKBANDIT_REVERSE_PROXY_PATH_BASE', '')
      .trim();
    if (!rawValue) {
      return '';
    }
  ...
  get seedDemoData(): boolean {
    return (
      this.configService.get<string>('TASKBANDIT_BOOTSTRAP_SEED_DEMO_DATA', 'false') === 'true'
    );
  }
  ...
  get composeFileHint(): string {
    return this.configService
      .get<string>('TASKBANDIT_COMPOSE_FILE_HINT', './docker-compose.yml')
      .trim();
  }
  ...
  get corsAllowedOrigins(): string[] {
    const explicitOrigins = this.configService
  ...
  get runtimeLogMaxFileSizeBytes(): number {
    ...
    const resolvedMegabytes =
      Number.isFinite(configuredValue) && configuredValue > 0 ? configuredValue : 100;
  ...
  get runtimeLogMaxTotalSizeBytes(): number {
    ...
    const resolvedMegabytes =
      Number.isFinite(configuredValue) && configuredValue > 0 ? configuredValue : 500;
    return Math.max(Math.round(resolvedMegabytes * 1024 * 1024), this.runtimeLogMaxFileSizeBytes);
  }
  ...
  get dockerLogMaxSize(): string {
    return (
      this.configService.get<string>('TASKBANDIT_DOCKER_LOG_MAX_SIZE', '100m').trim() || '100m'
    );
  }
  ...
  get fcmServiceAccount(): FirebaseServiceAccount | null {
    const rawJson = this.configService
      .get<string>('TASKBANDIT_FCM_SERVICE_ACCOUNT_JSON', '')
      .trim();
    const rawBase64 = this.configService
      .get<string>('TASKBANDIT_FCM_SERVICE_ACCOUNT_BASE64', '')
      .trim();
    const resolvedJson =
      rawJson || (rawBase64 ? Buffer.from(rawBase64, 'base64').toString('utf8') : '');
  ...
  get jwtSecret(): string {
    return this.configService.get<string>(
      ...
      'taskbandit-dev-secret-change-me',
    );
  }
  ...
  get forceLocalAuthEnabled(): boolean {
    return (
      this.configService.get<string>('TASKBANDIT_FORCE_LOCAL_AUTH_ENABLED', 'false') === 'true'
    );
  }
  ...
  get controlPlaneInternalServiceToken(): string {
    return this.configService
      .get<string>('TASKBANDIT_CONTROL_PLANE_INTERNAL_SERVICE_TOKEN', '')
      .trim();
  }
  ...
  get hostedRuntimeConfigCacheTtlMs(): number {
    ...
    return Number.isFinite(configuredValue) && configuredValue >= 1000 ? configuredValue : 60000;
  }
  ...
  get oidcFallbackConfig(): OidcConfig {
    ...
    const scope = this.configService
      .get<string>('TASKBANDIT_OIDC_SCOPE', 'openid profile email')
      .trim();
